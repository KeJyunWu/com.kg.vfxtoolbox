...
{ 
    class CombinedMesh : System.IDisposable
    {
        ...
        public CombinedMesh(IEnumerable<Mesh> meshes, IEnumerable<SkinnedMeshRenderer> sources)
        => Initialize( meshes.ToArray().Concat(sources.Select(smr => smr.sharedMesh).ToArray()).ToArray());
